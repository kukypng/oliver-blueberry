...
export const LicenseReportsPanel = () => {
  ...
  const fetchLicenseStats = async () => {
    try {
      const { data, error } = await supabase.rpc('admin_get_license_stats');
  ...
  const createBulkLicenses = async () => {
    ...
    try {
      const { data, error } = await supabase.rpc('admin_create_bulk_licenses', {
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="10"
              />
              <p className="text-xs text-muted-foreground">Máximo: 100 licenças</p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                max="3650"
                value={bulkDays}
                onChange={(e) => setBulkDays(parseInt(e.target.value) || 365)}
                placeholder="365"
              />
          ...
          >
            {isCreatingLicenses ? 'Criando...' : `Criar ${bulkQuantity} Licenças`}
          </Button>
        </CardContent>
      </Card>
